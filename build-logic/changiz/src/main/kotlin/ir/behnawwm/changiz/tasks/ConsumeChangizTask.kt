...
abstract class ConsumeChangizTask : DefaultTask() {
    ...
    fun consume() {
        ...
        val bumpType = entries.maxOf { it.type }
        if (bumpType == BumpType.NONE) {
            logger.lifecycle("All entries are type 'none'. No version bump needed.")
        ...
        config.languages.forEach { lang ->
            versionDir.resolve("public_$lang.txt").writeText(MarketRenderer.render(entries, lang))
            versionDir.resolve("internal_$lang.md").writeText(ChangelogRenderer.renderVersionInternal(entries, lang))
        }
        ...
        prependToFile(changelogFile, ChangelogRenderer.renderFullBlock(entries, newVersion, today))

        val publicChangelogFile = project.rootDir.resolve("changelogs/CHANGELOG_PUBLIC.md")
        prependToFile(publicChangelogFile, ChangelogRenderer.renderPublicBlock(entries, newVersion, today, config.languages))

        writeVersion(versionFile, newVersion)
        cleanupEntries(entries)

        logger.lifecycle("✅ Released $newVersion")
        logger.lifecycle("   Changelogs: changelogs/versions/$newVersion/")
        logger.lifecycle("   Updated: ${config.versionFile}, changelogs/CHANGELOG.md")
    }
    ...
    private fun prependToFile(file: File, content: String) {
        ...
        val header = existing.lines().first { it.startsWith("#") || it.isBlank() }
